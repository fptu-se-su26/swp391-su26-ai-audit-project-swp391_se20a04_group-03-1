...
export default function SpotTile({
  ...
  return (
    <button
      ...
      className={`relative flex h-full w-full flex-col justify-between rounded-[18px] border p-3 text-left transition-all duration-150 ${
        selected
    ...
    >
      ...
      <div className="text-[1.05rem] font-black tracking-wide md:text-xl">
        {spot.id}
      </div>
      <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.16em] opacity-80 md:text-[11px] md:tracking-[0.2em]">
        <span>
          {spot.status === "free"
            ? "trống"
            : spot.status === "occupied"
