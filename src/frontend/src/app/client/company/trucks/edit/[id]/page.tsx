...
export default function EditTruckPage({
    ...
    const fetchTruck = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/client/trucks/${resolvedParams.id}`,
          { credentials: "include" },
        );
        if (!res.ok) throw new Error("Không thể tải thông tin xe");
  ...
  useEffect(() => {
    ...
    validator
      ...
      .addField("#truckType", [{ rule: "required", errorMessage: "Bắt buộc." }])
      .onSuccess(async (event: any) => {
        ...
        try {
          ...
          if (data.code !== "success") {
            throw new Error(data.message || "Lỗi khi cập nhật xe.");
          }

          toast.success("Cập nhật xe thành công!", { id: loadingToast });
          setTimeout(() => router.push("/client/company/trucks"), 1500);
        } catch (err: any) {
          toast.error(err.message || "Không thể lưu thông tin xe.", { id: loadingToast });
        }
      });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#666666] dark:text-[#b3b3b3] font-bold mt-1">
            Cập nhật thông tin hồ sơ cho xe {truck?.truckType}
          </p>
        </div>
      </div>
      ...
      <Card className="bg-[#ffffff] dark:bg-[#181818] border-[#e5e5e5] dark:border-[#272727] rounded-[16px] shadow-sm overflow-visible">
        ...
        <CardContent className="pt-8">
          <form ref={formRef} id="truckEditForm" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                <Label htmlFor="truckPlate" className="text-[12px] font-bold uppercase tracking-[1.5px] text-[#121212] dark:text-[#ffffff]">
                  Biển số xe (*)
                </Label>
                <Input
                  id="truckPlate"
                  name="truckPlate"
                  defaultValue={truck?.truckPlate}
              ...
              <div className="space-y-3">
                <Label htmlFor="truckType" className="text-[12px] font-bold uppercase tracking-[1.5px] text-[#121212] dark:text-[#ffffff]">
                  Loại Xe
                </Label>
                <Input
                  id="truckType"
