...
import { ArrowLeft, Trash2, Box, Search, Loader2, RotateCcw, Archive } from "lucide-react";
...
export default function ProviderTrashContainersPage() {
  ...
  const fetchContainers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/provider/containers/trash?${params.toString()}`, {
        credentials: "include",
      });
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/provider/containers/restore/${id}`, {
        method: "PATCH",
        credentials: "include",
      });
      const data = await res.json();
  ...
  const handleHardDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/provider/containers/hard-delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#f3727f] font-bold mt-2 text-[12px] uppercase tracking-wider">
            Các container đã bị xóa sẽ nằm ở đây
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/client/provider/containers">
            <Button
              variant="outline"
              className="bg-[#ffffff] dark:bg-[#181818] border border-[#e5e5e5] dark:border-[#272727] text-[#121212] dark:text-[#ffffff] hover:bg-[#f8f8f8] dark:hover:bg-[#272727] hover:text-[#121212] dark:hover:text-[#ffffff] rounded-[500px] font-bold uppercase tracking-wider transition-colors gap-2 px-6 h-12"
            >
      ...
      <Card className="bg-[#ffffff] dark:bg-[#181818] border-[#e5e5e5] dark:border-[#272727] rounded-[16px] shadow-sm overflow-visible border-t-4 border-t-[#f3727f]">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative w-full sm:w-[220px]">
                ...
                <Input
                  ...
                  className="pl-11 bg-[#ffffff] dark:bg-[#121212] border border-[#e5e5e5] dark:border-[#272727] rounded-[500px] h-10 font-bold text-[14px] focus-visible:ring-0 focus-visible:border-[#f3727f] dark:focus-visible:border-[#f3727f] hover:border-[#f3727f] transition-colors"
                />
              </div>
        ...
        <CardContent className="p-0 overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              <div className="h-16 w-16 bg-[#f8f8f8] dark:bg-[#121212] rounded-full flex items-center justify-center mb-4">
                <Box className="h-8 w-8 text-[#999999]" />
              </div>
              <h3 className="text-lg font-black text-[#121212] dark:text-[#ffffff] uppercase tracking-wider mb-2">
                Thùng rác trống
              </h3>
              <p className="text-[#666666] dark:text-[#b3b3b3] font-medium max-w-md">
                Không có container nào trong thùng rác hoặc không có dữ liệu phù hợp với bộ lọc.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-[10px] text-[#666666] dark:text-[#999999] uppercase tracking-wider bg-[#f8f8f8] dark:bg-[#121212] border-b border-[#e5e5e5] dark:border-[#272727]">
                  <tr>
                    <th className="px-6 py-4 font-black">Mã BIC Container</th>
                    <th className="px-6 py-4 font-black">Loại & Kích thước</th>
                ...
                <tbody className="divide-y divide-[#e5e5e5] dark:divide-[#272727]">
                  {containers.map((container) => (
                    ...
                    >
                      <td className="px-6 py-4 font-black text-[#121212] dark:text-[#ffffff] uppercase text-[15px] tracking-wide">
                        {container.number}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1.5 rounded-[4px] text-[10px] font-black uppercase tracking-wider ${
                        ...
                        <span
                          className={`px-3 py-1.5 rounded-[4px] text-[10px] font-black uppercase tracking-wider ${
                            container.portStatus === "Chưa nhập cảng"
                              ...
                              : container.portStatus === "Đã xuất cảng"
                            ...
                            <AlertDialogContent className="bg-[#ffffff] dark:bg-[#181818] border-[#e5e5e5] dark:border-[#272727] rounded-[16px]">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription className="text-[#666666] dark:text-[#b3b3b3] font-bold text-[14px]">
                                  ...
                                  <span className="text-[#f3727f] font-black">
                                    {container.number}
                                  </span>{" "}
                              ...
                              <AlertDialogFooter className="mt-6 gap-3">
                                <AlertDialogCancel className="bg-[#f8f8f8] dark:bg-[#121212] text-[#121212] dark:text-[#ffffff] border-[#e5e5e5] dark:border-[#272727] rounded-[500px] font-bold uppercase tracking-wider px-6 hover:bg-[#e5e5e5] dark:hover:bg-[#272727]">
                                  Hủy
                                </AlertDialogCancel>
                                <AlertDialogAction
        ...
        {!loading && totalPages > 1 && containers.length > 0 && (
          <div className="py-6 border-t border-[#e5e5e5] dark:border-[#272727] bg-[#f8f8f8] dark:bg-[#121212] flex justify-center rounded-b-[16px]">
            <Pagination>
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
