...
export default function ProviderContainersPage() {
  ...
  const fetchContainers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/provider/containers/?${params.toString()}`, {
        credentials: "include",
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/provider/containers/soft-delete/${id}`, {
        method: "PATCH",
        credentials: "include",
      });
      const data = await res.json();
      if (data.code === "success") {
        toast.success(data.message || "Đã xóa thành công.", { id: loadingToast });
        fetchContainers();
      } else {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#666666] dark:text-[#b3b3b3] font-bold mt-1 uppercase tracking-wider text-[12px]">
            Đăng ký và theo dõi container của hãng tàu
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/client/provider/containers/trash">
            <Button
              variant="outline"
              className="bg-[#ffffff] dark:bg-[#181818] border border-[#e5e5e5] dark:border-[#272727] text-[#121212] dark:text-[#ffffff] hover:bg-[#f8f8f8] dark:hover:bg-[#272727] hover:text-[#121212] dark:hover:text-[#ffffff] hover:border-[#f3727f] dark:hover:border-[#f3727f] rounded-[500px] font-bold uppercase tracking-wider transition-colors gap-2 px-6 h-12"
            >
      ...
      {/* Stats - using fetched data totalItems */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* Keeping other static for UI simplicity or calculate if needed */}
      </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              ...
              <p className="text-[#666666] dark:text-[#b3b3b3] font-medium max-w-md">
                Chưa có container nào phù hợp với bộ lọc hoặc bạn chưa đăng ký container nào trên hệ thống.
              </p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link href={`/client/provider/containers/edit/${container._id}`}>
                            <Button
                              title="Sửa"
