...
const sealData = [
  { id: 1, sealNo: "SEAL-001", container: "CNT-001", status: "Bình thường", tempC: 22, humidity: 65, lastUpdate: "2026-05-19 14:30", condition: "Tốt" },
  { id: 2, sealNo: "SEAL-002", container: "CNT-002", status: "Bình thường", tempC: 23, humidity: 68, lastUpdate: "2026-05-19 14:25", condition: "Tốt" },
  { id: 3, sealNo: "SEAL-003", container: "CNT-003", status: "Cảnh báo", tempC: 28, humidity: 75, lastUpdate: "2026-05-19 14:20", condition: "Cao" },
  { id: 4, sealNo: "SEAL-004", container: "CNT-004", status: "Bình thường", tempC: 21, humidity: 62, lastUpdate: "2026-05-19 14:15", condition: "Tốt" },
]
    ...
    <div className="space-y-6">
      ...
      {showAlert && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-yellow-900">Cảnh báo nhiệt độ cao</h3>
            <p className="text-sm text-yellow-800">Container CNT-003 có nhiệt độ vượt ngưỡng (28°C). Vui lòng kiểm tra.</p>
          </div>
          <button onClick={() => setShowAlert(false)} className="ml-auto text-yellow-600 hover:text-yellow-700">✕</button>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-3 px-4 font-medium">Niêm phong</th>
                  ...
                  <th className="text-left py-3 px-4 font-medium">Cập nhật lần cuối</th>
                  <th className="text-left py-3 px-4 font-medium">Hành động</th>
                </tr>
              </thead>
              <tbody>
                {sealData.map((seal) => (
                  <tr key={seal.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                        ...
                        {seal.status === "Bình thường" ? <CheckCircle className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}
                        {seal.status}
                      </span>
                    </td>
                    <td className="py-3 px-4">{seal.tempC}°C</td>
                    <td className="py-3 px-4">{seal.humidity}%</td>
                    <td className="py-3 px-4 text-xs text-slate-500">{seal.lastUpdate}</td>
                    <td className="py-3 px-4">
