...
export default function ClientRolesTrashPage() {
  ...
  const fetchRoles = async (page = 1) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/settings/client-roles/trash?${queryParams}`, {
        credentials: "include"
      });
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/settings/client-roles/${id}/restore`, {
        method: "PATCH",
        credentials: "include",
      });
      const data = await res.json();
      ...
      toast.error("Không thể khôi phục loại hình", { id: loadingToast });
    }
  };
  ...
  const handleHardDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/settings/client-roles/${id}/hard-delete`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        <Link href="/admin/settings/client-roles">
        ...
        <div>
          ...
          <p className="text-[#666666] dark:text-[#b3b3b3] mt-2 text-[16px]">
            Quản lý và khôi phục các loại hình doanh nghiệp đã bị xóa.
          </p>
        </div>
      </div>
                    ...
                    <td colSpan={4} className="px-6 py-20">
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        <p className="font-bold text-[#666666] dark:text-[#b3b3b3] text-[16px]">
                          {searchQuery ? "Không tìm thấy loại hình nào phù hợp trong thùng rác." : "Thùng rác trống."}
                        </p>
                      </div>
                    </td>
                  </tr>
                            ...
                            <AlertDialogContent className="bg-[#ffffff] dark:bg-[#181818] border-[#e5e5e5] dark:border-[#272727] rounded-[16px]">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription className="text-[#666666] dark:text-[#b3b3b3] font-bold text-[14px]">
                                  Bạn có chắc chắn muốn xóa vĩnh viễn loại hình doanh nghiệp{" "}
                                  <span className="text-[#f3727f]">{role.roleName}</span>? Hành động này không thể hoàn tác.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter className="mt-6 gap-3">
                                ...
                                <AlertDialogAction
                                  onClick={() => handleHardDelete(role._id)}
