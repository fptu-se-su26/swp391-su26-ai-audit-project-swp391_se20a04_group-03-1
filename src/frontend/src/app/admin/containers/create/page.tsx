...
export default function CreateContainerPage() {
  ...
  useEffect(() => {
    if (selectedProvider) {
      ...
      if (provider && provider.bic_codes) {
        ...
        if (provider.bic_codes.length > 0) {
  ...
  useEffect(() => {
    ...
    validator
      ...
      .onSuccess(async (event: any) => {
        ...
        if (!selectedBic) {
          toast.error("Hãng tàu chưa cấu hình mã BIC hoặc bạn chưa chọn.");
          return;
        }
        ...
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/containers/create`, {
            method: "POST",
  ...
  }, [formType, formStatus, formPortStatus, selectedProvider, selectedBic, router]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#666666] dark:text-[#b3b3b3] mt-2 text-[16px]">
            Thêm container mới vào hệ thống
          </p>
        </div>
