...
export default function AdminContainersPage() {
  ...
  const fetchContainers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/containers/?${params.toString()}`, {
        credentials: "include",
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/containers/soft-delete/${id}`, {
        method: "PATCH",
        credentials: "include",
      });
      const data = await res.json();
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#666666] dark:text-[#b3b3b3] font-bold mt-1 uppercase tracking-wider text-[12px]">
            Quản lý toàn bộ container trong hệ thống
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/admin/containers/trash">
            <Button
              variant="outline"
                            ...
                            <AlertDialogContent className="bg-[#ffffff] dark:bg-[#181818] border-[#e5e5e5] dark:border-[#272727] rounded-[16px]">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription className="text-[#666666] dark:text-[#b3b3b3] font-bold text-[14px]">
                                  Bạn có chắc chắn muốn chuyển container{" "}
                                  <span className="text-[#121212] dark:text-[#ffffff] font-black">
                                    {container.number}
                                  </span>{" "}
                                  vào thùng rác?
                                </AlertDialogDescription>
