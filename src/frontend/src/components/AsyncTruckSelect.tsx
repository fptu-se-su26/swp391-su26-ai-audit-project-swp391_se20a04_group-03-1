...
export function AsyncTruckSelect({ 
    value, 
    onChange, 
    error 
}: { 
    ...
    useEffect(() => {
        const fetchTrucks = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/trucks?search=${encodeURIComponent(debouncedSearch)}&limit=20`, { credentials: "include" });
