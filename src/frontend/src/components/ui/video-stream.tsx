    ...
    >
      ...
      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
        ...
        {!hasError && streamUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-3 p-4 select-none">
            ...
            <div className="text-center">
              <p className="text-slate-400 text-sm font-medium">Mất tín hiệu camera</p>
              <p className="text-slate-500 text-xs mt-0.5">Camera ID: {cameraId}</p>
            </div>
            <Button 
              size="sm" 
              variant="outline" 
        ...
        <div className="absolute top-3 left-3 pointer-events-none select-none">
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-white text-[10px] font-bold tracking-wider uppercase rounded-md shadow-sm ${
            !hasError ? "bg-red-600 animate-none" : "bg-slate-700"
          }`}>
            <span className={`w-1.5 h-1.5 bg-white rounded-full ${!hasError && "animate-ping"}`} />
