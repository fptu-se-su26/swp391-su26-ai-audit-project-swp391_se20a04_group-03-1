...
export function AsyncCompanyContainerSelect({ 
    value, 
    onChange, 
    error 
}: { 
    ...
    useEffect(() => {
        const fetchContainers = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/containers?search=${encodeURIComponent(debouncedSearch)}&limit=20`, { credentials: "include" });
