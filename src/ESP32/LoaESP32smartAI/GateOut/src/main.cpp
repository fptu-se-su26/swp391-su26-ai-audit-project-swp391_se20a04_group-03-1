...
void moveServoSmoothly(int startAngle, int targetAngle) {
  int stepAngle = 30; // Chạy từng nấc 30 độ
  int delayMs = 300;  // Dừng 300ms ở mỗi nấc để kiểm soát góc

  if (startAngle < targetAngle) {
    for (int angle = startAngle + stepAngle; angle < targetAngle; angle += stepAngle) {
      gateServo.write(angle);
      delay(delayMs);
    }
    // Chạy nấc cuối cùng để đảm bảo đến đúng góc đích
...
// --- HÀM KIỂM TRA TRẠNG THÁI HỒNG NGOẠI CÓ LỌC NHIỄU (DEBOUNCE) ---
// Giúp chống nhiễu (tránh lỗi liên tục do ánh sáng hoặc cảm biến chập chờn)
bool isIRStateStable(int expectedState, int stableTimeMs) {
  unsigned long startTime = millis();
  while (millis() - startTime < stableTimeMs) {
    if (digitalRead(IR_PIN) != expectedState) {
      return false; // Bị nhiễu, trạng thái không giữ được
    }
    delay(5); // Chờ 1 chút rồi đọc lại để tránh block CPU hoàn toàn
  }
  return true; // Trạng thái đã giữ ổn định trong khoảng stableTimeMs
}
...
void setup() {
  ...
  Serial.println("GateOut Ready! Cho 5 giay de mo cong...");
}
...
void loop() {
  if (!isGateOpen) {
    ...
    if (digitalRead(IR_PIN) == IR_CAR_PRESENT) {
      if (isIRStateStable(IR_CAR_PRESENT, 100)) { // Phải giữ trạng thái CÓ XE liên tục 100ms
        ...
        while (true) {
          if (digitalRead(IR_PIN) == IR_NO_CAR) {
            if (isIRStateStable(IR_NO_CAR, 500)) { // Phải giữ trạng thái KHÔNG CÓ XE liên tục 500ms
              break; // Xe đã qua hẳn, thoát vòng lặp
            }
          }
