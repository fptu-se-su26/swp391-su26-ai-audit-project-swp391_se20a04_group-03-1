 ...
 *   - 1 GateStaff: gate.demo@logiport.vn / 123456 (Active).
...
async function run() {
  ...
  // 3. GateStaff
  let staff = await GateStaff.findOne({ email: GATE_EMAIL });
  if (!staff) {
    staff = await GateStaff.create({
      fullName: "Trần Quản Cổng",
      email: GATE_EMAIL,
      password: hashed,
      status: "Active",
    });
    console.log("[seed-mobile] Tạo GateStaff:", GATE_EMAIL);
  } else {
    staff.password = hashed;
    staff.status = "Active";
    await staff.save();
    console.log("[seed-mobile] Cập nhật GateStaff demo (reset mật khẩu)");
  }
